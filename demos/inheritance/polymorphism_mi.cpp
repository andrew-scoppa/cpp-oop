...
public:
    Shape(Point it = Point{0, 0}, int rotation = 0) : _loc(it), _rotation(rotation)
    {
...
public:
    Circle(double radius = 1, Point it = Point{0, 0}) : _radius(radius), Shape::Shape(it)
    {
    }
...
int main()
{
    ...
    Shape *pShapes[] = {new Circle(10), new Rectangle(50)};

    const int count = sizeof(pShapes) / sizeof(Shape*);
    for (Shape **it = pShapes; it < (pShapes + count) ; ++it)
    {
        (*it)->Draw();
    }
